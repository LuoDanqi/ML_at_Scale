...
# MAGIC > __a)__ The first order condition for convexity can make sure if there is a local maximum/minimum value, and the second order condition help us to prove that the local maximum/minimum value is also global maximum/minimum value. This is important when performing Gradient Descent, because we will need to know if the optimal value is the global optimal value.
# MAGIC 
# MAGIC > __b)__ The prolem domain space is the space of the true regression function where the model parameter space is the space of the gression loss functon (MSE in OLS case) and parameters.  The error surface in model parameter space is really hard to visualize in real life, with 2 or 3 parameters, we can still visualize in 2-D or 3D space, but with higher dimensions, it will be hard to see. Also, even by looking at the error surface cannot prove the local optimal value could be the global optimal value, we still need to mathematically prove by using first and second derivative functions.
# MAGIC 
# MAGIC > __c)__ Learning rate is the small increament value for each loop of updating paremeter. Increasing the learning rate reduces the number of iterations with which GD converges. However, a large learning rate might make the algorithm ends up overshooting the minimum and never converging at all!  
# MAGIC 
# MAGIC > __d)__ Calculating the optimal value by solving the loss functions. Chances are the loss function could be extreamly complicated. For high deminsions, it will be hard to parellize. Another reason is, when solving inverting X'X matrix, the covariance matrix cannot be solved if there are colinearity of two or more variables.
...
# MAGIC > __a)__ Based on the sampling data, most of the features are skewed distributed, except for the pH feature is close to normaly distributed. In terms of variance, alchol seems varies the most and chlorides varies the least. The variable color is a categorical feature with 0 and 1, in the regression coefficient, color will be a dummy variable, value 0 will be the base model and value 1 will be comparsion to the base model.  
# MAGIC 
# MAGIC > __b)__ From the sample plots showing here. It is really hard to see a linear relationship with high correlation, the critic acid is the only one has positive linear relationship with quality; and the volatile acidity has negative linear relationship with quality.
# MAGIC 
# MAGIC > __c)__ Based on the correlation matrix, paris of feautures with most strongly associated with each others are: 1. volatile acidity and color, 2. free sulfur dioxide and total sulfur dioxide 3. density and alcholo, 4. total sulfur and color. We only need to pick the ones with less multicollinearity for feature independency. We can also take a look at the R values to make the selection.   
...
# MAGIC > __b)__ The loss function will be 
# MAGIC $$
# MAGIC f(\boldsymbol{\theta}) = \frac{1}{n}\sum\_{i=1}^{n}\left[ \boldsymbol{\theta}^T\cdot\mathbf{x}'\_i - y\_i\right]^2  \ \ \ \ \ \ 
# MAGIC $$
# MAGIC graphically, it is the sum of squred residuals between the actual value and prediction over the size of the training data n.
# MAGIC 
# MAGIC > __c)__ We augment a data point in linear models and vector for convenience by considering the bias term \\(b\\) as weight \\(w\_{m+1}\\) and adding an input X of additional 1 at \\(X\_{m+1}\\).  
# MAGIC 
# MAGIC > __d)__ Yes, it is "embarassingly parallel". We can broadcast the wights and caluclated residuals per row basis. The aggregation, which is averaging all the residuals, can be done in the reduce phase. 
# MAGIC 
# MAGIC > __e)__ Yes, as we see the variance from part a is 0.76, which matches the result in part e. Because we are inputing 0.0 for all coefficients, which means no weights were added and the predictions will always be the meanQuality, therefore, the loss function became the same to variance formula.  

# COMMAND ----------

# part a - mean and variance of the outcome variable
# trainRDDCached.map(lambda x: x[1]).stats()
...
def OLSLoss(dataRDD, W):
    ...
    ################## YOUR CODE HERE ##################
    loss = augmentedData.map(lambda x: (W.T.dot(x[0])-x[1])**2).mean()
    ################## (END) YOUR CODE ##################
    return loss
...
# has a bias term equal to the mean of your outcome variable and 0.0 for all coefficients.
BASELINE = np.append([meanQuality], [0]*len(trainRDDCached.take(1)[0][0]))
...
# MAGIC > __a)__ The gradient:\\(\nabla f(\boldsymbol{\theta}) = \frac{2}{B}\sum\_{i=1}^{B}\left(\boldsymbol{\theta}^T \cdot\mathbf{x}'\_{\_i} - y\_{\_i}\right)\cdot \mathbf{x}'\_{\_i} \\) , the term \\(\boldsymbol{\theta}^T \cdot\mathbf{x}'\_{\_i} - y\_{\_i}\\) is the mean of error as weight, which is multiplied by the data \\(\mathbf{x}'\_{\_i}\\).
# MAGIC 
# MAGIC 
# MAGIC > __b)__ Calculating the data wieghted by the error,  \\(\boldsymbol{\theta}^T \cdot\mathbf{x}'\_{a\_i} - y\_{a\_i}\\) can be paralleized as mapping. Reducer can calculate the mean after aggregating the values. After reducing, the new \\({\theta}\\) will be updated by the gradient. 
# MAGIC 
# MAGIC > __c)__ _complete the coding portions of this question before answering d & e_ 
# MAGIC 
# MAGIC > __d)__ The loss instead of decreasing to a smaller value it gets larger and larger, not converging. 
# MAGIC 
# MAGIC > __e)__ Before normalization, since each feature has different scale of value, the gradient will take longer steps to get the optimal value. Therefore, after normalization, the contours become less skewed and easier to find the optimal value.   
...
def GDUpdate(dataRDD, W, learningRate = 0.1):
    ...
    ################## YOUR CODE HERE ################# 
#     grad = 2*augmentedData.map(lambda x: (W.T.dot(x[0])-x[1]).dot(x[0])).mean()
    grad = 2*augmentedData.map(lambda x: (x[1] - W.T.dot(x[0]))*x[0]).mean()
...
trainRDDCached.map(lambda x: x[0]).mean()
...
# MAGIC > __a)__ Because without knowing the minimum loss, we don't know if we found the MSE, the only thing we know is smaller loss gets us closer to the best OLS funtion.  
# MAGIC 
# MAGIC > __c)__ The training loss is much lower than the test loss. This indicates the overfitting on the training set.
# MAGIC 
# MAGIC > __d)__ When set seed = 5, the test loss and train loss are very close to each other; when set seed = 4, the test loss actually is smaller than the train loss, however, the train loss is higher than seed = 5 plot. The way we initialze the seed value to randomly split the dataset into training and testing can change the results of the model. The possible reason why it changes so much might because the orignial dataset is ordered. 
...
def GradientDescent(trainRDD, testRDD, wInit, nSteps = 20, 
    ...
    for idx in range(nSteps): 
        ...
        ############## YOUR CODE HERE #############
#         model = GDUpdate(normalize(trainRDD), model)
#         training_loss = OLSLoss(normalize(trainRDD), model)
#         test_loss = OLSLoss(normalize(testRDD), model)
...
# MAGIC > __a)__ 5 models x 50 iterations = 250 passes
# MAGIC 
# MAGIC > __b)__ For each record in the orignial dataset, 5 models x 2 (train + test) =10 tuples will be generated. The keys are string key which contains "model number" and "test or train set". We will be able to split the keys and get train loss and test loss for each model for averaging the total loss.  
# MAGIC 
# MAGIC > __e)__ The cross validation can help generalization of the model, as we can see from the plot of test error and train error.
...
def CVLoss(dataSplits, models):
    ...
    ################ YOUR CODE HERE #################    
#     loss = partialLossRDD.aggregateByKey((0,0), lambda a, b: (a[0] + b, a[1]+1), lambda a,b: (a[0] + b[0], a[1] + b[1]))\
...
# MAGIC > __a)__ Ridge loss function: 
# MAGIC $$
# MAGIC f(\boldsymbol{\theta}) = \frac{1}{n}\sum\_{i=1}^{n}\left[ \boldsymbol{\theta}^T\cdot\mathbf{x}'\_i - y\_i\right]^2 + \lambda\sum\_{i=1}^m (\theta\_{i})^2 \ \ \ \ \ \ 
# MAGIC $$
# MAGIC The gradient:
# MAGIC $$
# MAGIC \nabla f(\boldsymbol{\theta}) = \frac{2}{B}\sum\_{i=1}^{B}\left(\boldsymbol{\theta}^T \cdot\mathbf{x}'\_{\_i} - y\_{\_i}\right)\cdot \mathbf{x}'\_{\_i} + 2\lambda\sum\_{i=1}^m (\theta\_{i}) \ \ \ \ \ \ 
# MAGIC $$
# MAGIC The extra step adds a penalty that scales with the size of the magnitude of the estimated weights / coefficients. In doing so, it seeks to regulate overfitting, multi-collinearity, and the overall intuitive concept of adding too many unnecessary regressors for the sake of incremental improvement.
# MAGIC 
# MAGIC > __b)__ Lasso loss function: 
# MAGIC $$
# MAGIC f(\boldsymbol{\theta}) = \frac{1}{n}\sum\_{i=1}^{n}\left[ \boldsymbol{\theta}^T\cdot\mathbf{x}'\_i - y\_i\right]^2 + \lambda\sum\_{i=1}^m |\theta\_{i}| \ \ \ \ \ \ 
# MAGIC $$
# MAGIC The gradient:
# MAGIC $$
# MAGIC \nabla f(\boldsymbol{\theta}) = \frac{2}{B}\sum\_{i=1}^{B}\left(\boldsymbol{\theta}^T \cdot\mathbf{x}'\_{\_i} - y\_{\_i}\right)\cdot \mathbf{x}'\_{\_i} + \lambda\sum\_{i=1}^m |\theta\_{i}|/(\theta\_{i}) \ \ \ \ \ \ 
# MAGIC $$  
# MAGIC The extra step in Lasso, like Ridge, adds a penalty that scales with the size of the magnitude of the estimated weights / coefficients. In doing so, it seeks to regulate overfitting, multi-collinearity, and the overall intuitive concept of adding too many unnecessary regressors for the sake of incremental improvement. Lasso tends to force coefficients to absolute 0, as opposed to Ridge.
# MAGIC 
# MAGIC > __d)__ First of all, both L1 and L2 regulations makes the train loss and test loss very close to each other. For L2 regulations, ridge tend to give us a a more stable or smooth curve and L1 regulation, lasso give us a zig-zag line of loss. This is because of lasso tend to reverse the sign between positive and negative which makes the loss value less smooth.
...
def GDUpdate_wReg(dataRDD, W, learningRate = 0.1, regType = None, regParam = 0.1):
    ...
    new_model = None
    #################### YOUR CODE HERE ###################
    gradient = -2*augmentedData.map(lambda x: (x[1] - W.T.dot(x[0]))*x[0]).mean()
    regW = W.copy()
    regW[0] = 0 # make the first term, bias term, to be 0
    if regType == 'ridge':
      grad = gradient + 2*regParam*regW
    elif regType == 'lasso':
      grad = gradient + regParam*np.sign(regW)
    else:
      grad = gradient
...
def GradientDescent_wReg(trainRDD, testRDD, wInit, nSteps = 20, learningRate = 0.1,
    ...
    for idx in range(nSteps):  
        ...
        # keep track of test/train loss for plotting
        if regType == 'ridge':
          training_loss = OLSLoss(trainRDD, model) + regParam*np.sum(np.square(model[1:])) # update train loss remove the bias term
          test_loss = OLSLoss(testRDD, model) + regParam*np.sum(np.square(model[1:]))# update test loss remove the bias term
        elif regType == 'lasso':
          training_loss = OLSLoss(trainRDD, model) + regParam*np.sum(np.absolute(model[1:])) # update train loss remove the bias term
          test_loss = OLSLoss(testRDD, model) + regParam*np.sum(np.absolute(model[1:])) # update test loss remove the bias term
        else:
            training_loss = OLSLoss(trainRDD, model) # update train loss
            test_loss = OLSLoss(testRDD, model) # update test loss
...
# MAGIC > __b)__ Original OLS model performed best. Overall, we see that the best OLS model from the 50 iterations produced a mean squared error that was less than the mean squared error for either the Ridge or Lasso models. For next step, updating hyperparameter `lambda` for the Lasso and Ridge regularization to see if there is better results. Or we can increase the iterations to see if we reaches the global optiaml value.
# MAGIC 
# MAGIC > __c)__ There appears to be a linear relationship between the residuals and the quality. If the models are predicting correctly, we would expect to see a flat line along 0. In general, we see that we underrate wine quality for lower quality wines, overrate wine quality for higher quality wines. It might caused by grading bias in validation set.
# MAGIC 
# MAGIC > __d)__ For the most part, coefficients in each of the models increases and stablizes as the iteration increases, which is caused by the loss function minimization. The OLS and Ridge regression coefficients plateau generally smoothly and consistently. The Lasso regression coefficients increases with some turbulence, especially noticeable for those near 0, which is expected and consistent with the MSEs we saw previously, and the fact that Lasso tends to push coefficient values to 0. The more extreme coefficients in Lasso are also more extreme than those seen in the other regressions. 
...
validationRDD = heldOutRDDCached.map(lambda x: ((x[0]-featureMeans)/featureStdev,x[1]))
