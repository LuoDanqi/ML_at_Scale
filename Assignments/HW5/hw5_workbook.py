...
# MAGIC %md
# MAGIC 
# MAGIC #### Group members who has worked together on homework 5:
# MAGIC 
...
# RUN THIS CELL AS IS. 
# sum is a function, used total instead
total = 0
...
# MAGIC > __a)__ A subway station map can be represented as a graph. Stations can be represnted as nodes while tracks between stations can be represented as edges. Subway maps are directed graphs, and they tend to have the average in-degree of 2 to 3 for a station in many countries. A high in-degree of a station means that the station has many tracks comming in from its neighboring stations, and therefore, is potentially crowded. 
# MAGIC 
# MAGIC > __b)__ Each step of graph traversal requires updates of some metadata that should be shared across worker nodes. Since MapReduce does not provide an efficient mechanism for exchanging global data, graph traversal could be challenging. Also, if a graph is dense and large, keeping the entire graph structure over all worker nodes could be challenging.
# MAGIC 
# MAGIC > __c)__ Dijsktra's algorithm sovles the single-source shortest path problem, where the task is to find shortest paths from a source node to all other nodes in the graph. Priority queue is the central data structure of Dijkstra's algorithm, but the data structure should be maintained globally across all worker nodes, which makes it hard to parallelize this algorithm by using MapReduce. Also, the algorithm needs to proceed sequentially, and therefore, it is hard to parallelize.
# MAGIC 
# MAGIC > __d)__ Instead of using a global priority queue that tracks graph travelsal frontier, the parallel BFS iterates a sufficiently large number of times so that shortest paths are all intentified at the end. The parallel BFS does not track visited nodes, and therefore, traverses a graph redundantly.   
...
# MAGIC > __a)__  The graph is sparse becuase it does not have many edges; 9 out of 25 are zero in the adjacency matrix. Dense graphs have many ones in the adjacency matrix while sparce graphs don't. If the graph is sparse, adjacency lists save storage space, and therefore, more suitable. In contrast, dense graphs are normally represented with adjacency matrices, becuase the storage benefit of using adjacency lists diminishes as the number of edges increases.
# MAGIC 
# MAGIC > __b)__  The graph is directed. The adjacency matrices for undirected graphs are symmetric, so only the half of the matrix is required. In contrast, the adjacency matrix of directed graphs are normally asymmetric because of its directionality.  
...
def get_adj_matr(graph):
    ...
    for start, end in graph['edges']:
      adj_matr[end][start] = 1 # Here is backwards
    ############### (END) YOUR CODE #################
    return adj_matr
...
# MAGIC > __a)__ The PageRank metric represents the proportion of time a random-walk surfer spends on each webpage. 
# MAGIC 
# MAGIC > __b)__ The Markov Property indicates memoryless, meaning that the next state depends only on the current state and not on the sequence of events that preceded it. This means that PageRanks can be computed in a iterative way, Power iteration method, where the next state is calculated only based on the previous state.  
# MAGIC 
# MAGIC > __c)__ Each state corresponds to each webpage in the WebGraph. Assuming the number of webpages in the given WebGraph is large, the size of the transition matrix will be large.  Weights for edges of the graph represent probabilities of transition from source to destination, therefore, the size of transition probability matrix is  \\(n\times n\\).
# MAGIC 
# MAGIC > __d)__ A right stochastic matrix is a square matrix of nonnegative real numbers, with each row summing to 1. The left eigenvector of a right stochastic matrix, or transition matrix in our context, is the stationary distribution, or PageRanks, whose eigen value is one and all components are nonnegative.
# MAGIC 
# MAGIC > __e)__ It takes about 56 iterations to converge, and the node E is the most central. This makes sense because E has the highest in-degree of 2, and all its neighbors (B and D) also have the highest in-degree of 2, meaning that E is referenced by many popular webpages. 
...
################ YOUR CODE HERE #################
transition_matrix =(TOY_ADJ_MATR.T / TOY_ADJ_MATR.sum(axis=1)).T # TOY_ADJ_MATR / TOY_ADJ_MATR.sum(axis=1)
...
def power_iteration(xInit, tMatrix, nIter, verbose = True):
    ...
    state_vector = None
    ################ YOUR CODE HERE #################
    # Power iteration xP^k = alpha
    for i in range(nIter+1):
      state_vector = xInit.dot(np.linalg.matrix_power(tMatrix, i))
      if verbose:
         print("--"*20)
         print("step:", i)
         print(state_vector)
...
states = power_iteration(xInit, transition_matrix, 20, verbose = True)
...
# MAGIC > __a)__ Because of a dangling node E, the sum of probabilities of all nodes diminishes after each iteration. Specifically, the total probability mass gets smaller and smaller with more iterations because the probability mass that goes into E will not come out and disappear.  
# MAGIC 
# MAGIC > __b)__ E is the dangling node, and the corresponding row of the transition matrix is all zero, which makes the probability assigned to the node E lost in the next Power Iteration. To prevent this loss, we can redistribute the dangling nodes' mass to all nodes in the graph after each iteration.
# MAGIC 
# MAGIC > __c)__ There should be a path for every node to other nodes in the graph to be irreducible. Webgraph is not natrually irreducible. There could potentially be some pages that do not reference other pages but are referenced a lot, that's how some people manipulated the page ranks in the 1990s.
# MAGIC 
# MAGIC > __d)__ A graph is aperiodic if the period, or greatest common divisor, of all cycle lenghts is 1. For instance, self-loops create aperiodicity. The webgraph is not naturally aperiodic, and a modification is required to apply Power Iteration method to compute PageRanks. This is why we adopt a random jump factor that enables a web-surfer to come back to the same web page.
# MAGIC 
# MAGIC > __e)__ Redistribution of the dangling nodes mass and teleportation are applied to make the webgraph transition matrix irreducible and aperiodic. Redistribution of dangling nodes mass ensures the total mass of the graph is kept to 1.0, and teleporting simulates surfers jumping to a random webpage at the next iteration, including the current webpage itself. 
...
transition_matrix2 = (TOY2_ADJ_MATR.T / TOY2_ADJ_MATR.sum(axis=1)).T
transition_matrix2.loc["E",] = 0 # set E has no outlet
# transition_matrix2.loc["E",] = 1/transition_matrix2.shape[1] # set E to be evenly distributed
...
# MAGIC > __a)__  Each record of the raw dataset is a pair of a non-dangling node's ID in the given graph and its adjacency lists. The first value is the node id, and the second value is the adjacency list for the node in a dictionary format.  
# MAGIC 
# MAGIC > __b)__ Since dangling nodes are missing in the raw dataset, the number of records is not equal to the number of total nodes in the graph. 
# MAGIC 
# MAGIC > __d)__ The total number of nodes is 15192277, and the number of non-dangling nodes is 5781290. Therefore, there are 9410987 (15192277 - 5781290)  dangling nodes.
...
def count_nodes(dataRDD):
    ...
    ############## YOUR CODE HERE ###############
    def emitNodes(x):
      node, neighbors = x.split("\t")
      yield node
      for x in ast.literal_eval(neighbors).keys():
        yield x
      return
...
# MAGIC 
# MAGIC > __b)__ A node's out-degree is used to calculate the distributed probabilities to the node's adjacent nodes. Specifically, the out-degree of a node is the denominator of probabilities for its all out-going edges. 
# MAGIC 
# MAGIC > __c)__ When a node's out-degree is 0, the node is dangling. We should redistribute dangling nodes' mass over all nodes in the graph for each iteration. 
...
def count_degree(dataRDD, n):
    ...
    ############## YOUR CODE HERE ###############
    result = dataRDD.map(parse).map(lambda x: (x[0], sum(x[1].values()))).cache()

    top = result.takeOrdered(10, key=lambda x: -x[1])
    avgDegree = result.map(lambda x: x[1]).mean()
    sampledCounts = result.map(lambda x: x[1]).takeSample(False, n, 42) 
...
# MAGIC 
# MAGIC > __a)__ N is the total number of nodes, or websites. The initial rank of 1/N for every node assumes that a web-surfer will start her web-surfing journey by randomly choosing a website out of N websites.
# MAGIC 
# MAGIC > __b)__ Each record should have 1/N as its initial PageRank, and therefore, it is more efficient to compute N first and then use 1/N for all nodes' records. Otherwise, we will need to update N to all nodes to get the probability when calcualting the dangling nodes number.
...
def initGraph(dataRDD):
    ...
    def emitNodesWithMass(x):
      """
      output two types of records. 
      1. Node with edges but no mass (graph structure)
      2. Neighbor node with mass but no edges (distribution)
      3. dangling node can be determined by empty edges 
      """
      node, edges = x.split("\t")
    ...
    def mergeEdges(x, y):
      """
      add the edges in the dictionary by node
      """
      x.update(y)
      return x 
 
    # write your main Spark code here
    graphRDD = dataRDD.flatMap(emitNodesWithMass).reduceByKey(mergeEdges).cache()
    N = graphRDD.count()
    graphRDD = graphRDD.map(lambda x: (x[0], (1/N, x[1])))
...
# MAGIC 
# MAGIC > __a)__  1/|G| represents the random jump factor which assumes a web-surfer will teleport to a node randomly from any node. The alpha term is the probability that teleporting occurs. The two terms multiplied indicates the likelihood of a web surfer to arrive at a specific node via random jump. 
# MAGIC 
# MAGIC > __b)__ \\( m \\)  represents the sum of PageRank credits assigned to all dangling nodes. At the end of each iteration, we uniformly redistribute \\( m \\) to all nodes in the graph by adding the \\( {m} / {|G|} \\) term to each node's PageRank. 
# MAGIC 
...
def runPageRank(graphInitRDD, alpha = 0.15, maxIter = 10, verbose = True):
    ...
    def distMass(x):
      """
        input: a pair RDD of (node_id , (score, edges))
      """
      node_id, score, edges = x[0], x[1][0], x[1][1] 
      total_weight = sum(edges.values())
      yield (node_id, (0, edges))

      for x in edges.keys():
        yield (x, (score*edges[x]/total_weight, dict()))
      return
    ...
    def bonusMass(x):
      #new_score = a.value*(1/N) + d.value*(mm.value/n.value + x[1][0])
      new_score = a.value*(1/n.value) + d.value*(mm.value/n.value + x[1][0])
      return (x[0], (new_score, x[1][1]))
    ...
    for i in range(maxIter):
      steadyStateRDD.foreach(lambda x: mmAccum.add(x[1][0]) if len(x[1][1]) == 0 else None)
      steadyStateRDD = steadyStateRDD.flatMap(distMass).reduceByKey(mergeEdges).cache()
      N = steadyStateRDD.count()
      n = sc.broadcast(N)
      mm = sc.broadcast(mmAccum.value)

      steadyStateRDD = steadyStateRDD.map(bonusMass).cache()
      steadyStateRDD.foreach(lambda x: totAccum.add(x[1][0]))

      print(f"[{i}th iteration] mmAccum:{mmAccum}, totAccum:{totAccum}")

      mmAccum = sc.accumulator(0.0, FloatAccumulatorParam())
      totAccum = sc.accumulator(0.0, FloatAccumulatorParam())
    
    steadyStateRDD = steadyStateRDD.map(lambda x: (x[0], x[1][0])).cache()
...
top_20_DF = top_20_RDD.toDF()
top_20_DF = top_20_DF.withColumnRenamed('_1','id2')
top_20_DF = top_20_DF.withColumnRenamed('_2','PageRank')
joinedWithNames = namesKV_DF.join(top_20_DF, namesKV_DF.id==top_20_DF.id2).drop('id2').orderBy(F.desc("PageRank")).collect()
############## END YOUR CODE ###############
...
for r in joinedWithNames:
    print ("{:6f}\t| {:10d}\t| {}".format(r[2],r[0],r[1]))
...
# Modified here, same to the code below
